...
export const OneGraph = ({ graphData }: OneGraphType) => {
    ...
    return (
        <article className={oneGraphStyles.allGraphs} >
            <canvas key={Math.random()} ref={canvasRef} className={oneGraphStyles.canvas} />
            <canvas key={Math.random()} ref={canvas_pointerRef} className={oneGraphStyles.canvas_pointer} />
        </article>
    )
}